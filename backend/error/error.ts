...
export const error_handler:ErrorHandler = async (err, c) => {
  ...
  if (e instanceof Custom_HTTP_Error){
    return c.json({ code: e.status, message: e.message, name: e.name }, e.status);
  }else{
    if (e.status === undefined || e.message === undefined){
      e = {status:500, message:"Internal Server Error"} as HTTPException
    }
    return c.json({ code: e.status, message: e.message }, e.status);
  }  
}
